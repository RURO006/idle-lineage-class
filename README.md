...
| 更新時間   | 版號      | 更新內容                                                                                   |
| ---------- | --------- | ------------------------------------------------------------------------------------------ |
| 2026-09-15 | `v3.8.59` | 將登入、創角、戰鬥、技能特效、NPC、寵物、圖鑑、潘朵拉與城戰的動態圖片預載及逐幀切換整合 `GetCacheUrl`，同一頁面內改用 Blob URL 快取並保留原始圖片 fallback。 |
| 2026-09-15 | `v3.8.58` | 修正進入角色選擇畫面後首頁登入動畫仍持續切換圖片，避免 `assets/login` 動畫資源被不必要地反覆載入。 |
| 2026-09-15 | `v3.8.57` | 修正登入角色後選角動畫仍在背景持續切換圖片，避免 `assets/start` 動畫資源被不必要地反覆載入。 |
| 2026-09-14 | `v3.8.56` | 優化 Google 雲端登入：頁面載入時先自動嘗試靜默恢復授權，登入與雲端同步改為靜默登入優先，只有必要時才顯示授權視窗。 |
| 2026-09-14 | `v3.8.55` | 調整雷德的復仇任務五種證明戒指掉落率：精靈使、喚獸師、黑法師、黑暗棲林者與馴獸師戒指統一提高至 0.2%。 |
| 2026-09-14 | `v3.8.54` | 修正主角色使用隊員已分配的試煉道具完成任務後，隊員進度帳未同步扣除，導致隊員後續無法再取得任務物品。 |
| 2026-09-14 | `v3.8.53` | 調整進化果實配方：光明的鱗片需求量由 5 片降為 2 片。                              |
| 2026-09-14 | `v3.8.52` | 調整寵物魅力需求為 4／6／8／10；寵物魅力傷害與命中加成公式由 `魅力×0.1` 提升為 `Math.floor(有效魅力×0.3)`。 |
| 2026-09-14 | `v3.8.51` | 調整開始畫面排版：主選單垂直置中並限制內容高度，匯出／匯入所有進度按鈕改為並排顯示，改善不同視窗尺寸下的完整性與可用性。 |
| 2026-09-14 | `v3.8.50` | 將 Google 雲端儲存／讀取合併為「雲端同步」；記錄上次同步的雲端版本，偵測多台裝置衝突後交由使用者選擇同步方向，並修正執行期角色世代造成的假差異。 |
| 2026-09-14 | `v3.8.49` | 將六大屬性的升級配點與回憶蠟燭重配上限由 60 提升至 100。 |
| 2026-09-14 | `v3.8.48` | 寵物進化規則改成自動升到100等再進化。世界頻道 NPC 新增寵物 CP 問答；玩家詢問寵物 CP／性價比／排行／輸出／血量時，回覆 Lv.60 輸出與血量各 50% 的比較清單。 |
| 2026-09-13 | `v3.8.47` | 倉庫雙側清單新增物品圖示並依分類排序；不適用目前角色的裝備會以黯淡／暗紅狀態顯示。 |
| 2026-09-13 | `v3.8.46` | 修正替換或卸下傭兵裝備後，退回隊長物品欄的裝備未立即顯示，現在會即時刷新背包。 |
